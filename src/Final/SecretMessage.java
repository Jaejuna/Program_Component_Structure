...
public class SecretMessage {
	...
	public String encode(String plain, String key) {
		...
		for (int i = 0; i < text.length; i ++) {
			for (int j = 0; j < text.length; j++) {
				board[i][j] = text[plain], pickOneRandomly(key);
			}
		}
		...
		String encoded = "";
		for (int i = 0; i < text.length; i ++)
			encoded = encoded + encode(plain, key);
		return encoded;
	}
	...
	public String decode(String secret, String key) {
		...
		for (int i = 0; i < text.length; i ++) {
			for (int j = 0; j < text.length; j++) {
				board[i][j] = text[secret], pickOneRandomly(key);
			}
		}
		...
		String decoded = "";
		for (int i = 0; i < text.length; i ++)
			decoded = decoded + decode(isInKey(secret), ' ');
		return decoded;
	}
